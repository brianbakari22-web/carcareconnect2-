    ...
    <div>
      ...
      <div style={{ position:"relative", background:"#0f0f0f", border:"1px solid #1e1e1e", borderRadius:isMobile?10:16, overflow:"hidden", marginBottom:"1rem" }}>
        ...
        <div style={{ position:"relative", zIndex:1, padding:isMobile?"1rem":"2rem 1.5rem", display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:10 }}>
          <div>
            ...
            <div style={{ fontFamily:"Syne", fontSize:isMobile?18:26, fontWeight:800, color:"#f0ede6", marginBottom:2 }}>
              Car<span style={{ color:"#e6821e" }}>Care</span> Connect
            </div>
            <div style={{ fontSize:11, color:"#555" }}>Admin Control Center · Nairobi</div>
          </div>
        ...
        <div style={{ position:"relative", zIndex:1, display:"grid", gridTemplateColumns:"repeat(2,1fr)", borderTop:"1px solid #1e1e1e" }}>
          {[
            ...
            <div key={s.label} style={{ padding:isMobile?"0.6rem 0.9rem":"0.9rem 1.25rem", borderRight:i%2===0?"1px solid #1e1e1e":"none", borderBottom:i<2?"1px solid #1e1e1e":"none" }}>
              <div style={{ fontSize:9, color:"#555", textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:3 }}>{s.label}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:8, marginBottom:"1rem" }}>
        {[
          ...
          <div key={s.label} style={{ background:"#111", borderRadius:10, padding:isMobile?"0.7rem":"1rem", border:"1px solid #1e1e1e" }}>
            <div style={{ fontSize:9, color:"#555", textTransform:"uppercase", letterSpacing:"0.05em", marginBottom:3 }}>{s.label}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:8, marginBottom:"1rem" }}>
        <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1rem" }}>
          <div style={{ fontFamily:"Syne", fontSize:13, fontWeight:700, marginBottom:2, color:"#f0ede6" }}>Booking trend</div>
          <div style={{ fontSize:10, color:"#555", marginBottom:"0.75rem" }}>Per month</div>
            ...
            {bookingTrend.map(m=>(
              <div key={m.month} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
                ...
                <div style={{ width:"100%", display:"flex", flexDirection:"column", gap:1 }}>
                  <div style={{ width:"100%", background:"#f0ede6", borderRadius:"2px 2px 0 0", height:`${Math.max(2,(m.completed/maxBookings)*50)}px` }}/>
                  <div style={{ width:"100%", background:"#555", height:`${Math.max(2,(m.pending/maxBookings)*50)}px` }}/>
                  <div style={{ width:"100%", background:"#333", height:`${Math.max(m.cancelled>0?2:0,(m.cancelled/maxBookings)*50)}px` }}/>
                </div>
                <div style={{ fontSize:7, color:"#555", textAlign:"center" }}>{shortMonth(m.month)}</div>
              </div>
            ))}
          </div>
          <div style={{ display:"flex", gap:8, marginTop:8, flexWrap:"wrap" }}>
            {[{label:"Done",bg:"#f0ede6"},{label:"Pending",bg:"#555"},{label:"Cancelled",bg:"#333"}].map(l=>(
        ...
        <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1rem" }}>
          <div style={{ fontFamily:"Syne", fontSize:13, fontWeight:700, marginBottom:2, color:"#f0ede6" }}>User growth</div>
          <div style={{ fontSize:10, color:"#555", marginBottom:"0.75rem" }}>Per month</div>
          ...
          <div style={{ display:"flex", gap:6, alignItems:"flex-end" }}>
            {userGrowth.map(m=>(
              <div key={m.month} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:2 }}>
                <div style={{ fontSize:8, color:"#666", fontWeight:600 }}>{m.total}</div>
                <div style={{ width:"100%", display:"flex", flexDirection:"column", gap:1 }}>
                  <div style={{ width:"100%", background:"#f0ede6", borderRadius:"2px 2px 0 0", height:`${Math.max(2,(m.customers/maxUsers)*50)}px` }}/>
                  <div style={{ width:"100%", background:"#888", height:`${Math.max(2,(m.providers/maxUsers)*50)}px` }}/>
                  <div style={{ width:"100%", background:"#444", height:`${Math.max(m.drivers>0?2:0,(m.drivers/maxUsers)*50)}px` }}/>
                </div>
                <div style={{ fontSize:7, color:"#555", textAlign:"center" }}>{shortMonth(m.month)}</div>
              </div>
            ))}
      ...
      {/* Activity feed */}
      <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1rem" }}>
        <div style={{ fontFamily:"Syne", fontSize:13, fontWeight:700, marginBottom:"0.75rem", color:"#f0ede6", display:"flex", alignItems:"center", gap:6 }}>
          <div style={{ width:7, height:7, borderRadius:"50%", background:"#e6821e", boxShadow:"0 0 6px #e6821e" }}/>
          Live activity
        </div>
        {loading&&<div style={{ color:"#555", fontSize:12 }}>Loading...</div>}
        {!loading&&activity.length===0&&<div style={{ color:"#444", fontSize:12 }}>No activity yet</div>}
        {activity.map((a,i)=>(
          <div key={i} style={{ display:"flex", alignItems:"center", gap:10, padding:"7px 0", borderBottom:"1px solid #1a1a1a" }}>
            <div style={{ width:28, height:28, borderRadius:7, background:"#1a1a1a", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, flexShrink:0 }}>{a.icon}</div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:11, color:"#f0ede6", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{a.text}</div>
              <div style={{ fontSize:9, color:"#555", marginTop:1 }}>{a.sub}</div>
            </div>
            <div style={{ flexShrink:0, textAlign:"right" }}>
              <div style={{ fontSize:9, padding:"2px 6px", borderRadius:8, background:"#1a1a1a", color:"#888", marginBottom:2 }}>{a.tag}</div>
              <div style={{ fontSize:9, color:"#444" }}>{new Date(a.time).toLocaleTimeString([],{hour:"2-digit",minute:"2-digit"})}</div>
            </div>
          </div>
        ))}
      </div>
