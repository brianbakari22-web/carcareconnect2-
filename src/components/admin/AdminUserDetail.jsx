    ...
    <div>
      ...
      <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1.25rem", marginBottom:"1.25rem" }}>
        <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:"1.25rem", flexWrap:"wrap" }}>
          ...
          <div style={{ display:"flex", gap:8, flexShrink:0, flexWrap:"wrap" }}>
            <button onClick={handleExport} disabled={exporting}
              style={{ background:"#071a12", border:"1px solid #1d9e7540", borderRadius:8, color:"#1d9e75", fontSize:12, padding:"7px 14px", cursor:"pointer" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:isMobile?"repeat(2,1fr)":"repeat(4,1fr)", gap:10 }}>
          {[
            ...
            <div key={s.label} style={{ background:"#0f0f0f", borderRadius:8, padding:"0.75rem", border:"1px solid #1a1a1a" }}>
              <div style={{ fontSize:10, color:"#555", textTransform:"uppercase", marginBottom:4 }}>{s.label}</div>
              <div style={{ fontFamily:"Syne", fontSize:16, fontWeight:800, color:s.color||"#f0ede6" }}>{s.value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {tab==="overview"&&(
        <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:10 }}>
          ...
          <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1.25rem" }}>
            ...
            {!bookings.length&&<div style={{ color:"#444", fontSize:12 }}>No bookings</div>}
          </div>
        </div>
      )}
      ...
      {tab==="bookings"&&(
        <div>
          ...
          {!bookings.length&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No bookings</div>}
        </div>
      )}
      ...
      {tab==="payments"&&(
        <div>
          ...
          {!payments.length&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No payments</div>}
        </div>
      )}
      ...
      {tab==="reviews"&&(
        <div>
          ...
          {!reviews.length&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No reviews</div>}
        </div>
      )}
      ...
      {tab==="tickets"&&(
        <div>
          ...
          {!tickets.length&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No support tickets</div>}
        </div>
      )}
      ...
      {tab==="activity"&&(
        <div>
          ...
          {!notifications.length&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No activity</div>}
        </div>
      )}
    </div>
  )
