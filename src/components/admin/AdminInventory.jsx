  ...
  return (
    <div>
      <div style={{ marginBottom:"1.25rem" }}>
        <div style={{ fontFamily:"Syne", fontSize:isMobile?16:20, fontWeight:800, color:"#f0ede6" }}>Inventory Oversight</div>
        <div style={{ fontSize:12, color:"#555" }}>All provider inventory across platform</div>
      </div>
      ...
      <div style={{ display:"flex", gap:8, marginBottom:"1rem", flexWrap:"wrap" }}>
        ...
        <select value={providerFilter} onChange={e=>setProviderFilter(e.target.value)}
          style={{ background:"#111", border:"1px solid #222", borderRadius:8, padding:"8px 12px", color:"#f0ede6", fontSize:12, outline:"none" }}>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:"1rem", flexWrap:"wrap" }}>
        {CATS.map(c=>(
          <button key={c} onClick={()=>setCatFilter(c)}
            style={{ padding:"5px 10px", borderRadius:7, border:"none", fontSize:11, cursor:"pointer", background:catFilter===c?"#8b5cf6":"#111", color:catFilter===c?"#fff":"#666" }}>
            {c}
          </button>
        ))}
      </div>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, flexWrap:"wrap" }}>
                ...
                {!item.is_active&&<span style={{ fontSize:10, color:"#555", background:"#1a1a1a", padding:"1px 6px", borderRadius:6 }}>Hidden</span>}
                {item.stock_quantity===0&&<span style={{ fontSize:10, color:"#e24b4a", background:"#1a0808", padding:"1px 6px", borderRadius:6 }}>Out of stock</span>}
                {item.stock_quantity>0&&item.stock_quantity<=5&&<span style={{ fontSize:10, color:"#e6821e", background:"#1a1208", padding:"1px 6px", borderRadius:6 }}>⚠️ Low: {item.stock_quantity}</span>}
              </div>
              <div style={{ fontSize:11, color:"#555", marginBottom:2 }}>{item.category} {item.subcategory?`· ${item.subcategory}`:""}</div>
              ...
              {item.compatible_cars?.length>0&&<div style={{ fontSize:10, color:"#444", marginTop:2 }}>🚗 {item.compatible_cars.join(", ")}</div>}
            </div>
            <div style={{ textAlign:"right", flexShrink:0 }}>
              <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:800, color:"#e6821e" }}>KES {Number(item.price).toLocaleString()}</div>
              <div style={{ fontSize:11, color:"#555" }}>{item.stock_quantity} {item.unit}s</div>
            </div>
          </div>
        </div>
      ))}
