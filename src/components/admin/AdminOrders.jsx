  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1.25rem", flexWrap:"wrap", gap:10 }}>
        <div>
          <div style={{ fontFamily:"Syne", fontSize:isMobile?16:20, fontWeight:800, color:"#f0ede6" }}>Orders Management</div>
          <div style={{ fontSize:12, color:"#555" }}>Parts and accessories orders across platform</div>
        </div>
        <button onClick={()=>setShowZones(!showZones)}
          style={{ background:"#160a2e", border:"1px solid #8b5cf640", borderRadius:9, color:"#8b5cf6", fontFamily:"Syne,sans-serif", fontSize:13, fontWeight:700, padding:"9px 16px", cursor:"pointer" }}>
          📍 Delivery zones
        </button>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:"1.5rem" }}>
        {[
          { label:"Total orders", value:orders.length, color:"#f0ede6" },
      ...
      {showZones&&(
        <div style={{ background:"#111", border:"1px solid #8b5cf630", borderRadius:12, padding:"1.25rem", marginBottom:"1.5rem" }}>
          ...
          <form onSubmit={addZone} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:8, marginBottom:"1rem", alignItems:"end" }}>
            <div><label style={{ fontSize:10, color:"#555", display:"block", marginBottom:3 }}>Zone name</label><input style={inp} value={zoneForm.name} onChange={e=>setZoneForm(f=>({...f,name:e.target.value}))} placeholder="e.g. Westlands" required/></div>
            <div><label style={{ fontSize:10, color:"#555", display:"block", marginBottom:3 }}>Base fee (KES)</label><input type="number" style={inp} value={zoneForm.base_fee} onChange={e=>setZoneForm(f=>({...f,base_fee:e.target.value}))} required/></div>
            <div><label style={{ fontSize:10, color:"#555", display:"block", marginBottom:3 }}>Per km (KES)</label><input type="number" style={inp} value={zoneForm.per_km_fee} onChange={e=>setZoneForm(f=>({...f,per_km_fee:e.target.value}))} required/></div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:8 }}>
            {zones.map(z=>(
              <div key={z.id} style={{ background:"#0f0f0f", borderRadius:8, padding:"0.75rem", border:"1px solid "+(z.is_active?"#1d9e7530":"#1e1e1e"), opacity:z.is_active?1:0.6 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div style={{ fontSize:12, fontWeight:600, color:"#f0ede6" }}>{z.name}</div>
                ...
                <div style={{ fontSize:11, color:"#555", marginTop:4 }}>Base: KES {Number(z.base_fee).toLocaleString()} · Per km: KES {Number(z.per_km_fee).toLocaleString()}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display:"flex", gap:6, marginBottom:"1rem", flexWrap:"wrap" }}>
        {["all","pending","confirmed","processing","ready","delivered","cancelled"].map(f=>(
          <button key={f} onClick={()=>setFilter(f)}
            style={{ padding:"5px 12px", borderRadius:7, border:"none", fontSize:11, cursor:"pointer", background:filter===f?(SC[f]||"#8b5cf6"):"#111", color:filter===f?"#fff":"#666" }}>
      ...
      {filtered.map(o=>(
        <div key={o.id} style={{ background:"#111", border:"1px solid "+(SC[o.status]||"#1e1e1e")+"30", borderRadius:10, padding:"1rem", marginBottom:8 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
            <div>
              ...
              <div style={{ fontSize:11, color:"#555" }}>🏪 {o.provider?.business_name||o.provider?.first_name}</div>
              ...
              <div style={{ fontSize:11, color:"#555" }}>{o.fulfillment_type==="delivery"?"🚚 "+o.delivery_address:"🏪 Pickup"}</div>
          ...
          <div style={{ background:"#0f0f0f", borderRadius:8, padding:"0.6rem", marginBottom:8 }}>
          ...
          {o.status==="ready"&&o.fulfillment_type==="delivery"&&!o.delivery_driver_id&&(
            <button onClick={()=>assignDriver(o.id)} style={{ background:"#0c1f2e", border:"1px solid #378add40", borderRadius:7, color:"#378add", fontSize:11, padding:"5px 12px", cursor:"pointer" }}>
              🚚 Assign driver
            </button>
          )}
        </div>
