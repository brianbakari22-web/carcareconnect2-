...
export default function Admin2FA() {
  ...
  const inp = { width:"100%", background:theme.bg, border:`1px solid ${theme.borderLight}`, borderRadius:8, padding:"12px 14px", color:theme.text, fontSize:14, outline:"none", fontFamily:"'DM Sans',sans-serif", letterSpacing:4, textAlign:"center", marginBottom:12 }

  if (step==="loading") return <div style={{ color:theme.textFaint, fontSize:13 }}>Loading...</div>

  return (
    <div style={{ maxWidth:480 }}>
      <div style={{ marginBottom:"1.5rem" }}>
        <div style={{ fontFamily:"Syne", fontSize:20, fontWeight:800, color:theme.text }}>Two-Factor Authentication</div>
      ...
      {step==="setup"&&(
        <div style={{ background:theme.bgCard, border:`1px solid ${theme.border}`, borderRadius:12, padding:"1.25rem" }}>
          <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, marginBottom:8, color:theme.text }}>Setup authenticator</div>
          <div style={{ fontSize:12, color:theme.textMuted, marginBottom:"1.5rem", lineHeight:1.6 }}>
            Use an authenticator app like <strong style={{ color:theme.text }}>Google Authenticator</strong> or <strong style={{ color:theme.text }}>Authy</strong> to generate time-based codes for login verification.
          </div>
          {[
      ...
      {step==="scan"&&(
        <div>
          <div style={{ background:theme.bgCard, border:`1px solid ${theme.border}`, borderRadius:12, padding:"1.25rem", marginBottom:"1rem" }}>
            <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, marginBottom:"1rem", color:theme.text }}>Scan QR code</div>
            <div style={{ display:"flex", justifyContent:"center", marginBottom:"1rem" }}>
              {qrUrl&&<img src={qrUrl} alt="QR Code" style={{ width:180, height:180, borderRadius:10, border:"4px solid #fff" }}/>}
            </div>
            <div style={{ fontSize:11, color:theme.textFaint, textAlign:"center", marginBottom:"1rem" }}>
              Can't scan? Enter this code manually:
            </div>
            <div style={{ background:theme.bg, border:`1px solid ${theme.borderLight}`, borderRadius:8, padding:"0.75rem", fontFamily:"monospace", fontSize:13, color:"#e6821e", letterSpacing:3, textAlign:"center", wordBreak:"break-all", marginBottom:"1rem" }}>
              {secret}
            </div>
          </div>

          <div style={{ background:theme.bgCard, border:`1px solid ${theme.border}`, borderRadius:12, padding:"1.25rem" }}>
            <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, marginBottom:4, color:theme.text }}>Verify code</div>
            <div style={{ fontSize:12, color:theme.textMuted, marginBottom:"1rem" }}>Enter the 6-digit code from your authenticator app</div>
      ...
      {step==="backup"&&(
        <div style={{ background:theme.bgCard, border:"1px solid #1d9e7540", borderRadius:12, padding:"1.25rem" }}>
          <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, marginBottom:4, color:"#1d9e75" }}>✓ 2FA enabled!</div>
          <div style={{ fontSize:12, color:theme.textMuted, marginBottom:"1.25rem", lineHeight:1.6 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:6, marginBottom:"1.25rem" }}>
            {backupCodes.map((code,i)=>(
              <div key={i} style={{ background:theme.bg, border:`1px solid ${theme.borderLight}`, borderRadius:7, padding:"8px 12px", fontFamily:"monospace", fontSize:13, color:theme.text, textAlign:"center", letterSpacing:2 }}>
                {code}
              </div>
            ))}
          </div>
      ...
      {step==="enabled"&&(
        <div>
          <div style={{ background:theme.bgCard, border:`1px solid ${theme.border}`, borderRadius:12, padding:"1.25rem", marginBottom:"1rem" }}>
            <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, marginBottom:"1rem", color:theme.text }}>Backup codes</div>
          ...
          <div style={{ background:theme.bgCard, border:"1px solid #e24b4a20", borderRadius:12, padding:"1.25rem" }}>
            <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, marginBottom:4, color:"#e24b4a" }}>Disable 2FA</div>
            <div style={{ fontSize:12, color:theme.textMuted, marginBottom:"1rem" }}>
