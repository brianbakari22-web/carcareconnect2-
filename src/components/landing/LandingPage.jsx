...
export default function LandingPage() {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#ffffff", color:"#000000", fontFamily:"DM Sans,sans-serif", overflowX:"hidden", position:"relative" }}>
      ...
      {/* NAV */}
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:200, padding:"0.6rem 1rem", display:"flex", justifyContent:"space-between", alignItems:"center", background:"#000000", backdropFilter:"blur(12px)", borderBottom:"1px solid #1e1e1e", transition:"all 0.3s" }}>
      ...
      <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"5rem 1rem 3rem", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ position:"relative", maxWidth:820, zIndex:1 }}>
          ...
          <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", marginBottom:"2.5rem" }}>
            ...
            <button onClick={()=>navigate("/auth")} style={{ background:"none", border:"2px solid #e6821e", borderRadius:12, color:"#000000", fontFamily:"Syne,sans-serif", fontSize:16, fontWeight:700, padding:"14px 38px", cursor:"pointer" }}>
              Sign in →
            </button>
          </div>
          <div style={{ display:"flex", gap:8, justifyContent:"center", flexWrap:"wrap", marginBottom:"1.5rem" }}>
            {[["24/7","Emergency service"],["5min","Avg response"],["100%","Verified providers"],["🇰🇪","Made in Kenya"]].map(([v,l])=>(
              <div key={l} style={{ textAlign:"center", padding:"0.6rem 1rem", background:"#141414", borderRadius:10, border:"1px solid #1e1e1e" }}>
                <div style={{ fontFamily:"Syne", fontSize:18, fontWeight:800, color:"#000000" }}>{v}</div>
                <div style={{ fontSize:10, color:"#555555", marginTop:2 }}>{l}</div>
              </div>
            ))}
          </div>
          <div style={{ display:"flex", gap:20, justifyContent:"center", flexWrap:"wrap" }}>
            {[["🚗","Service booking"],["🚗","24/7 emergency"],["📍","Live tracking"],["🛒","Marketplace"],["🎁","Loyalty rewards"]].map(([icon,label])=>(
              <div key={label} style={{ fontSize:12, color:"#555555", display:"flex", alignItems:"center", gap:6 }}><span>{icon}</span><span>{label}</span></div>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ position:"relative", zIndex:1, margin:"0 1.5rem 5rem", background:"linear-gradient(135deg,#13080a,#1a0c0e)", border:"1px solid #e24b4a25", borderRadius:20, padding:"3rem 2rem", textAlign:"center", overflow:"hidden" }}>
        ...
        <h2 style={{ fontFamily:"Syne", fontSize:"clamp(22px,4vw,36px)", fontWeight:800, color:"#000000", marginBottom:12 }}>
        ...
        <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", marginBottom:"1.5rem" }}>
          {[["🛞","Flat tyre"],["🔋","Dead battery"],["⛽","Out of fuel"],["🌡","Overheating"],["🚚","Towing"]].map(([icon,label])=>(
            <div key={label} style={{ background:"#e24b4a15", border:"1px solid #e24b4a30", borderRadius:8, padding:"6px 12px", fontSize:12, color:"#000000" }}>{icon} {label}</div>
          ))}
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))", gap:20 }}>
          {[
            ...
            <div key={s.n} className="hcard" style={{ background:"#141414", border:"1px solid #1e1e1e", borderRadius:16, padding:"2rem", position:"relative", overflow:"hidden" }}>
              ...
              <div style={{ fontFamily:"Syne", fontSize:18, fontWeight:700, color:"#000000", marginBottom:10 }}>{s.title}</div>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1000, margin:"0 auto 5rem", padding:"0 1.5rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:16 }}>
          {[
            { icon:"🔧", title:"Service booking", desc:"Oil change, brakes, AC, full diagnostics from verified Nairobi providers", color:"#000000" },
            { icon:"🚗", title:"GO Service", desc:"24/7 emergency roadside. Mechanic dispatched to your GPS location fast", color:"#000000" },
            ...
            { icon:"🎁", title:"Loyalty rewards", desc:"Earn points every booking. Redeem for discounts on future services", color:"#555555" },
            { icon:"📍", title:"Live tracking", desc:"Track your driver or mechanic on a live dark map in real time", color:"#000000" },
            { icon:"🚨", title:"Service guarantee", desc:"Not happy? We investigate and issue a full service voucher refund", color:"#1d9e75" },
            { icon:"🤖", title:"24/7 assistant", desc:"Always-on help for car problems, bookings, and platform guidance", color:"#555555" },
          ].map(f=>(
            <div key={f.title} className="hcard" style={{ background:"#141414", border:"1px solid #1e1e1e", borderRadius:14, padding:"1.25rem" }}>
              ...
              <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, color:"#000000", marginBottom:6 }}>{f.title}</div>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1000, margin:"0 auto 5rem", padding:"0 1.5rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16 }}>
          {[
            ...
            { icon:"📍", title:"Real-time tracking", desc:"Know exactly where your mechanic or driver is at all times. Live GPS tracking on every job.", color:"#000000" },
            { icon:"🔐", title:"Secure payments", desc:"All payments processed through Pesapal, regulated by the Central Bank of Kenya.", color:"#555555" },
            { icon:"🤖", title:"24/7 support", desc:"Always-on support to help diagnose car problems, guide bookings, and answer any question.", color:"#000000" },
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1000, margin:"0 auto 5rem", padding:"0 1.5rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:16 }}>
          {[
            { icon:"🚗", role:"Customer", color:"#000000", desc:"Book services, get emergency help, track your car, earn rewards", features:["Book car services","Emergency GO Service","Live mechanic tracking","Buy and sell vehicles"] },
            ...
            <div key={r.role} className="hcard" style={{ background:"#141414", border:"1px solid "+r.color+"30", borderRadius:16, padding:"1.75rem" }}>
              ...
              <div style={{ fontSize:12, color:"#555555", marginBottom:14, lineHeight:1.6 }}>{r.desc}</div>
              {r.features.map(f=>(
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1000, margin:"0 auto 5rem", padding:"0 1.5rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:12 }}>
          {[
            { icon:"🔧", type:"Garage/Mechanic", commission:"90%", color:"#000000" },
            { icon:"⚙️", type:"Parts Dealer", commission:"95%", color:"#378add" },
            { icon:"✨", type:"Accessories Shop", commission:"92%", color:"#555555" },
            { icon:"🛞", type:"Tyre Shop", commission:"94%", color:"#1d9e75" },
            { icon:"⚡", type:"Auto Electrician", commission:"88%", color:"#000000" },
            { icon:"🚿", type:"Car Wash", commission:"90%", color:"#378add" },
            { icon:"⚙️", type:"Panel Beater", commission:"85%", color:"#000000" },
            ...
            <div key={b.type} className="hcard" style={{ background:"#141414", border:"1px solid #1e1e1e", borderRadius:12, padding:"1.25rem", textAlign:"center" }}>
              ...
              <div style={{ fontFamily:"Syne", fontSize:13, fontWeight:700, color:"#000000", marginBottom:4 }}>{b.type}</div>
              <div style={{ fontSize:11, color:b.color, fontWeight:600 }}>Keep {b.commission}</div>
            </div>
          ))}
        </div>
        <div style={{ textAlign:"center", marginTop:"2rem" }}>
          <button onClick={()=>navigate("/auth")} style={{ background:"none", border:"2px solid #e6821e", borderRadius:12, color:"#000000", fontFamily:"Syne,sans-serif", fontSize:14, fontWeight:700, padding:"12px 32px", cursor:"pointer" }}>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:900, margin:"0 auto 5rem", padding:"0 1.5rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(190px,1fr))", gap:16 }}>
          {[
            { type:"Shop Standard", provider:"90%", platform:"10%", desc:"Customer brings car to your shop", color:"#000000" },
            { type:"Shop Premium", provider:"80%", platform:"20%", desc:"Your mechanic travels to customer", color:"#378add" },
            { type:"GO Service", provider:"85%", platform:"15%", desc:"Emergency roadside assistance", color:"#000000" },
            ...
            <div key={p.type} className="hcard" style={{ background:"#141414", border:"1px solid "+p.color+"30", borderRadius:14, padding:"1.5rem", textAlign:"center" }}>
              ...
              <div style={{ fontFamily:"Syne", fontSize:36, fontWeight:800, color:"#000000", marginBottom:4 }}>{p.provider}</div>
              <div style={{ fontSize:11, color:"#555555", marginBottom:10 }}>Your earnings</div>
              <div style={{ fontSize:11, color:"#000000", marginBottom:6 }}>Platform: {p.platform}</div>
              <div style={{ fontSize:11, color:"#555555", lineHeight:1.5 }}>{p.desc}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ background:"#141414", border:"1px solid #1e1e1e", borderRadius:20, padding:"2rem" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))", gap:12 }}>
            {[
              ...
              <div key={service} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"10px 12px", background:"#f5f5f5", borderRadius:8 }}>
                <span style={{ fontSize:12, color:"#6b6b6b" }}>{service}</span>
                <span style={{ fontSize:12, color:"#000000", fontWeight:600 }}>{price}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1000, margin:"0 auto 5rem", padding:"0 1.5rem" }}>
        <div style={{ background:"linear-gradient(135deg,#141414,#1a1a1a)", border:"1px solid #1e1e1e", borderRadius:20, padding:"2.5rem 2rem" }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr", gap:"2rem", alignItems:"center" }}>
            <div>
              <div style={{ fontSize:11, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.1em", color:"#000000", marginBottom:8 }}>Parts marketplace</div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:10 }}>
              {[
                ...
                <div key={f.title} style={{ background:"#f5f5f5", borderRadius:12, padding:"1rem", textAlign:"center" }}>
                  ...
                  <div style={{ fontFamily:"Syne", fontSize:13, fontWeight:700, color:"#000000", marginBottom:4 }}>{f.title}</div>
                  <div style={{ fontSize:11, color:"#555555" }}>{f.desc}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      <footer style={{ position:"relative", zIndex:1, borderTop:"1px solid #141414", padding:"2.5rem 1.5rem", textAlign:"center" }}>
        ...
        <div style={{ display:"flex", gap:24, justifyContent:"center", flexWrap:"wrap", marginBottom:16 }}>
          {[["Privacy Policy","/privacy"],["Terms of Service","/terms"],["Contact","mailto:carcareconnect254@gmail.com"],["Support","mailto:carcareconnect254@gmail.com"]].map(([label,href])=>(
            <a key={label} href={href} style={{ fontSize:12, color:"#555555", textDecoration:"none" }}>{label}</a>
          ))}
        </div>
        <div style={{ fontSize:12, color:"#000000", marginBottom:6 }}>🇰🇪 Nairobi's most trusted automotive platform</div>
        <div style={{ fontSize:11, color:"#000000" }}>© 2026 Car Care Connect · Nairobi, Kenya · 0113858966 · carcareconnect254@gmail.com</div>
