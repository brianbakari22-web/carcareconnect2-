...
function FloatingParts() {
  const items = [
    { id:0, emoji:"🛞", left:5, top:15, size:52, duration:7, delay:0, opacity:0.25 },
    { id:1, emoji:"⚙️", left:88, top:10, size:48, duration:9, delay:1, opacity:0.20 },
    { id:2, emoji:"🔧", left:15, top:75, size:40, duration:6, delay:2, opacity:0.18 },
    { id:3, emoji:"🚗", left:78, top:65, size:56, duration:8, delay:0.5, opacity:0.15 },
    { id:4, emoji:"🔋", left:45, top:5, size:38, duration:7, delay:1.5, opacity:0.20 },
    { id:5, emoji:"🛢️", left:92, top:45, size:44, duration:10, delay:3, opacity:0.18 },
    { id:6, emoji:"🔩", left:25, top:45, size:34, duration:5, delay:0, opacity:0.22 },
    { id:7, emoji:"⚡", left:60, top:80, size:42, duration:8, delay:2, opacity:0.20 },
    { id:8, emoji:"🪛", left:70, top:25, size:36, duration:6, delay:1, opacity:0.18 },
    { id:9, emoji:"🔑", left:35, top:90, size:34, duration:9, delay:4, opacity:0.15 },
    { id:10, emoji:"🛠️", left:50, top:55, size:38, duration:7, delay:2.5, opacity:0.12 },
    { id:11, emoji:"🏎️", left:8, top:40, size:46, duration:11, delay:1, opacity:0.10 },
  ]
  return (
    <div style={{ position:"fixed", inset:0, pointerEvents:"none", zIndex:0, overflow:"hidden" }}>
      <style>{`@keyframes float-up{0%{transform:translateY(0) rotate(0deg)}50%{transform:translateY(-30px) rotate(10deg)}100%{transform:translateY(0) rotate(0deg)}}`}</style>
      {items.map(p=>(
        <div key={p.id} style={{ position:"absolute", left:p.left+"%", top:p.top+"%", fontSize:p.size, opacity:p.opacity, animation:`float-up ${p.duration}s ease-in-out ${p.delay}s infinite` }}>
          {p.emoji}
        </div>
      ))}
    </div>
...
const HOW_IT_WORKS = [
  ...
  { n:"03", icon:"tracking", title:"Track and review", desc:"Track your mechanic live on the map. Rate your experience and earn loyalty points on every booking." },
]
...
const Divider = () => (
  <div style={{ display:"flex", justifyContent:"center", padding:"2rem 0", overflow:"hidden" }}>
    <div style={{ fontSize:22, whiteSpace:"nowrap", animation:"marquee 20s linear infinite", opacity:0.3 }}>
      {"🛞 ⚙️ 🔧 🚗 🔋 🛢️ 🔩 ⚡ 🪛 🔑 🛠️ 🏎️   ".repeat(3)}
    </div>
  </div>
)
...
export default function LandingPage() {
  ...
  const btnOutline = { background:"none", border:"2px solid #000", borderRadius:500, color:"#000", fontFamily:"Syne,sans-serif", fontSize:14, fontWeight:700, padding:"13px 28px", cursor:"pointer" }
  const btnOutlineWhite = { background:"none", border:"1px solid #333", borderRadius:500, color:"#fff", fontFamily:"Syne,sans-serif", fontSize:14, fontWeight:700, padding:"13px 28px", cursor:"pointer" }
    ...
    <div style={{ fontFamily:"DM Sans,sans-serif", background:"#fff", color:"#000", overflowX:"hidden" }}>
      ...
      <nav style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"0 5%", height:60, background:"#000", position:"sticky", top:0, zIndex:100 }}>
        ...
        <div style={{ display:"flex", gap:10 }}>
          <button onClick={()=>navigate("/auth")} style={{ ...btnOutlineWhite, fontSize:13, padding:"8px 18px" }}>Sign in</button>
      ...
      <section style={{ position:"relative", minHeight:"92vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", textAlign:"center", padding:"4rem 5%", zIndex:1 }}>
        ...
        <div style={{ display:"flex", gap:32, flexWrap:"wrap", justifyContent:"center", marginBottom:40 }}>
          {[["24/7","Emergency service"],["5min","Avg response"],["100%","Verified providers"],["KE","Made in Kenya"]].map(([v,l])=>(
        ...
        <div style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center" }}>
          {[["vehicles","Service booking"],["emergency","24/7 emergency"],["tracking","Live tracking"],["marketplace","Marketplace"],["referral","Loyalty rewards"]].map(([icon,label])=>(
            <div key={label} style={{ display:"flex", alignItems:"center", gap:6, background:"#f5f5f5", borderRadius:100, padding:"7px 14px" }}>
      ...
      <section style={{ padding:"5rem 5%", background:"#0a0a0a", position:"relative", zIndex:1 }}>
        <div style={{ maxWidth:900, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap", justifyContent:"center", marginBottom:40 }}>
            {["Flat tyre","Dead battery","Out of fuel","Overheating","Towing"].map(item=>(
              <div key={item} style={{ background:"#1a1a1a", border:"1px solid #333", borderRadius:100, padding:"8px 18px", fontSize:13, color:"#fff" }}>{item}</div>
            ))}
          </div>
      ...
      <section style={{ padding:"5rem 5%", background:"#fafafa", zIndex:1, position:"relative" }}>
        <div style={{ maxWidth:1100, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:16 }}>
            {FEATURES.map(f=>(
              <div key={f.title} style={{ background:"#fff", borderRadius:16, padding:"1.5rem", border:"1px solid #eeeeee", textAlign:"center" }}>
                <div style={{ display:"flex", justifyContent:"center", marginBottom:12 }}><CCCIcon name={f.icon} size={44}/></div>
                <div style={{ fontFamily:"Syne", fontSize:16, fontWeight:800, color:"#000", marginBottom:6 }}>{f.title}</div>
            ...
            {ROLES.map(r=>(
              <div key={r.role} style={{ background:"#fff", borderRadius:20, padding:"2rem", border:`2px solid ${r.color}20` }}>
                ...
                <ul style={{ listStyle:"none", padding:0, margin:"0 0 20px" }}>
                  {r.features.map(feat=>(
                    <li key={feat} style={{ fontSize:13, color:"#555", padding:"5px 0", borderBottom:"1px solid #f0f0f0", display:"flex", alignItems:"center", gap:8 }}>
                      <span style={{ color:r.color, fontWeight:700 }}>+</span> {feat}
                    </li>
                  ))}
                </ul>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:16 }}>
            {[
              ...
              <div key={p.cat} style={{ background:"#f8f8f8", borderRadius:16, padding:"1.5rem", textAlign:"center" }}>
                ...
                <div style={{ fontSize:12, color:"#aaa", marginBottom:8 }}>Your earnings · {p.sub}</div>
      ...
      <section style={{ padding:"5rem 5%", background:"#fff", zIndex:1, position:"relative" }}>
        <div style={{ maxWidth:1000, margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:12, marginBottom:32 }}>
            {[["partsDealer","Engine and mechanical"],["accessories","Car accessories"],["tyreShop","Tyres all brands"],["autoElectrician","Batteries and electrical"],["earnings","Oils and fluids"]].map(([icon,label])=>(
          ...
          <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", marginBottom:32 }}>
            {[["Pickup","Collect from shop"],["Delivery","CCC riders deliver"],["Verified shops","All shops checked"],["Secure pay","M-Pesa or card"]].map(([title,desc])=>(
              <div key={title} style={{ background:"#f5f5f5", borderRadius:12, padding:"12px 16px", textAlign:"center" }}>
                <div style={{ fontSize:13, fontWeight:600, color:"#000" }}>{title}</div>
      ...
      <section style={{ padding:"6rem 5%", background:"#000", textAlign:"center", zIndex:1, position:"relative" }}>
        ...
        <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
          <button onClick={()=>navigate("/auth")} style={btnOrange}>Get started free</button>
          <button onClick={()=>navigate("/auth")} style={btnOutlineWhite}>Sign in</button>
        </div>
      </section>
