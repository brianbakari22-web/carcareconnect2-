...
function FloatingParts() {
  const items = [
    { id:0, icon:"≡ƒ¢₧", left:5, top:15, size:52, duration:7, delay:0, opacity:0.35 },
    { id:1, icon:"ΓÜÖ∩╕Å", left:88, top:10, size:48, duration:9, delay:1, opacity:0.30 },
    { id:2, icon:"≡ƒöº", left:15, top:75, size:40, duration:6, delay:2, opacity:0.28 },
    { id:3, icon:"≡ƒÜù", left:78, top:65, size:56, duration:8, delay:0.5, opacity:0.25 },
    { id:4, icon:"≡ƒöï", left:45, top:5, size:38, duration:7, delay:1.5, opacity:0.30 },
    { id:5, icon:"≡ƒ¢ó∩╕Å", left:92, top:45, size:44, duration:10, delay:3, opacity:0.28 },
    { id:6, icon:"≡ƒö⌐", left:25, top:45, size:34, duration:5, delay:0, opacity:0.32 },
    { id:7, icon:"ΓÜí", left:60, top:80, size:42, duration:8, delay:2, opacity:0.30 },
    { id:8, icon:"≡ƒ¬¢", left:70, top:25, size:36, duration:6, delay:1, opacity:0.28 },
    { id:9, icon:"≡ƒöæ", left:35, top:90, size:34, duration:9, delay:4, opacity:0.25 },
    { id:10, icon:"≡ƒ¢á∩╕Å", left:50, top:55, size:38, duration:7, delay:2.5, opacity:0.22 },
    { id:11, icon:"≡ƒÅÄ∩╕Å", left:10, top:55, size:46, duration:11, delay:0.5, opacity:0.28 },
  ]
  return (
...
export default function LandingPage() {
  ...
  const providerTypes = [
    { icon:"≡ƒöº", type:"Garage/Mechanic", keep:"90%" },
    { icon:"ΓÜÖ∩╕Å", type:"Parts Dealer", keep:"95%" },
    { icon:"Γ£¿", type:"Accessories Shop", keep:"92%" },
    { icon:"≡ƒ¢₧", type:"Tyre Shop", keep:"94%" },
    { icon:"ΓÜí", type:"Auto Electrician", keep:"88%" },
    { icon:"≡ƒÜ┐", type:"Car Wash", keep:"90%" },
    { icon:"≡ƒö¿", type:"Panel Beater", keep:"85%" },
    { icon:"≡ƒ¬ƒ", type:"Auto Glass", keep:"88%" },
  ]
  const features = [
    { icon:"≡ƒöº", title:"Service booking", desc:"Oil change, brakes, AC, full diagnostics from verified Nairobi providers" },
    { icon:"≡ƒÜ¿", title:"GO Service", desc:"24/7 emergency roadside. Mechanic dispatched to your GPS location fast" },
    { icon:"≡ƒÜù", title:"Concierge delivery", desc:"We collect your car, service it, and return it to your door" },
    { icon:"≡ƒ¢Æ", title:"Parts marketplace", desc:"Buy genuine and aftermarket parts from verified Nairobi shops" },
    { icon:"≡ƒÄü", title:"Loyalty rewards", desc:"Earn points every booking. Redeem for discounts on future services" },
    { icon:"≡ƒôì", title:"Live tracking", desc:"Track your driver or mechanic on a live map in real time" },
    { icon:"≡ƒ¢í∩╕Å", title:"Service guarantee", desc:"Not happy? We investigate and issue a full service voucher refund" },
    { icon:"≡ƒñû", title:"24/7 assistant", desc:"Always-on help for car problems, bookings, and platform guidance" },
  ]
  const faqs = [
    ...
    <div style={{ fontFamily:"DM Sans,sans-serif", background:"transparent", color:"#000", minHeight:"100vh", position:"relative" }}>
      ...
      <div style={{ paddingTop:56, position:"relative", overflow:"hidden", minHeight:"92vh", display:"flex", flexDirection:"column", justifyContent:"center", zIndex:1 }}>
        ...
        <div style={{ padding:"3rem 1.25rem 2.5rem", maxWidth:620, margin:"0 auto", textAlign:"center", position:"relative", zIndex:2 }}>
          ...
          <div style={{ display:"inline-flex", alignItems:"center", gap:6, background:"rgba(240,253,244,0.9)", border:"1px solid #bbf7d0", borderRadius:20, padding:"5px 14px", marginBottom:"1.5rem" }}>
            ...
            <span style={{ fontSize:12, color:"#16a34a", fontWeight:600 }}>≡ƒç░≡ƒç¬ Built in Kenya Γö¼Γòû Serving Nairobi</span>
          </div>
          ...
          <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap", marginBottom:"2rem" }}>
            <button onClick={()=>navigate("/auth")} style={btnOrange}>≡ƒÜù Get started free</button>
            <button onClick={()=>navigate("/auth")} style={btnOutline}>Sign in ΓåÆ</button>
          </div>
        ...
        <div style={{ background:"rgba(0,0,0,0.9)", padding:"1.25rem", textAlign:"center", position:"relative", zIndex:2 }}>
          <div style={{ display:"flex", gap:16, justifyContent:"center", flexWrap:"wrap" }}>
            {[["≡ƒÜù","Service booking"],["≡ƒÜ¿","24/7 emergency"],["≡ƒôì","Live tracking"],["≡ƒ¢Æ","Marketplace"],["≡ƒÄü","Loyalty rewards"]].map(([icon,label])=>(
      ...
      <div style={sec("rgba(17,17,17,0.88)")}>
        ...
        <div style={{ maxWidth:580, margin:"0 auto", textAlign:"center", position:"relative", zIndex:2 }}>
          ...
          <p style={{ ...body, color:"#aaa", marginBottom:"1.5rem" }}>Kenya's only 24/7 emergency roadside service. Our certified mechanics come to your exact GPS location ΓÇö highway, parking lot, or home. Just KES 500 callout fee.</p>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", justifyContent:"center", marginBottom:"1.5rem" }}>
            {["≡ƒ¢₧ Flat tyre","≡ƒöï Dead battery","Γ¢╜ Out of fuel","≡ƒîí∩╕Å Overheating","≡ƒÜÜ Towing"].map(item=>(
          ...
          <button onClick={()=>navigate("/auth")} style={{ ...btnOrange, background:"#e24b4a", fontSize:15, padding:"14px 32px" }}>≡ƒÜ¿ Request emergency help</button>
        </div>
      </div>
      ...
      <div style={sec("rgba(255,255,255,0.82)")}>
        ...
        <div style={{ maxWidth:800, margin:"0 auto", position:"relative", zIndex:2 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:12 }}>
            {[
              { n:"01", icon:"Γëí╞Æ├╢├¼", title:"Find a service", desc:"Browse verified providers near you. Compare prices, read reviews, and pick the best mechanic for your car and budget." },
      ...
      <div style={sec("rgba(255,255,255,0.82)")}>
        ...
        <div style={{ maxWidth:800, margin:"0 auto", position:"relative", zIndex:2 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:12 }}>
            {[
              { icon:"≡ƒ¢í∩╕Å", title:"Service Guarantee", desc:"Not happy with the service? We investigate and issue a full refund voucher. No questions asked." },
              { icon:"Γ£à", title:"Verified providers only", desc:"Every mechanic, parts dealer and service provider is manually verified before listing on CCC." },
              { icon:"≡ƒôì", title:"Real-time tracking", desc:"Know exactly where your mechanic or driver is at all times. Live GPS tracking on every job." },
              { icon:"≡ƒöÉ", title:"Secure payments", desc:"All payments processed through Pesapal, regulated by the Central Bank of Kenya." },
              { icon:"≡ƒñû", title:"24/7 support", desc:"Always-on support to help diagnose car problems, guide bookings, and answer any question." },
              { icon:"≡ƒç░≡ƒç¬", title:"Made in Kenya", desc:"Built by Kenyans for Kenyan roads. We understand Nairobi traffic, local mechanics, and Kenyan cars." },
            ].map(t=>(
      ...
      <div style={sec("rgba(17,17,17,0.88)")}>
        ...
        <div style={{ maxWidth:800, margin:"0 auto", position:"relative", zIndex:2 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:12 }}>
            {[
              { icon:"≡ƒÜù", role:"Customer", color:"#e6821e", desc:"Book services, get emergency help, track your car, earn rewards", features:["Book car services","Emergency GO Service","Live mechanic tracking","Parts marketplace"] },
              ...
              { icon:"≡ƒÜù", role:"Concierge Driver", color:"#1d9e75", desc:"Pick up and deliver customer vehicles and parts, earn per delivery", features:["Accept deliveries","Parts delivery jobs","Live navigation","KES 200 allowance per trip"] },
            ].map(r=>(
      ...
      <div style={sec("rgba(255,255,255,0.82)")}>
        ...
        <div style={{ maxWidth:800, margin:"0 auto", position:"relative", zIndex:2 }}>
          <div style={{ textAlign:"center", marginBottom:"2rem" }}>
            ...
            <p style={{ ...body, maxWidth:480, margin:"0 auto" }}>Not just mechanics ΓÇö list any automotive business and reach thousands of customers in Nairobi</p>
          </div>
          ...
          <div style={{ textAlign:"center" }}>
            <button onClick={()=>navigate("/auth")} style={btnOutlineOrange}>Register your business ΓåÆ</button>
          </div>
        </div>
      </div>
      ...
      <div style={sec("rgba(255,255,255,0.82)")}>
        ...
        <div style={{ maxWidth:800, margin:"0 auto", position:"relative", zIndex:2 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:"1.5rem" }}>
            {[["ΓÜÖ∩╕Å","Engine and mechanical parts"],["Γ£¿","Car accessories"],["≡ƒ¢₧","Tyres all brands"],["≡ƒöï","Batteries and electrical"],["≡ƒ¢ó∩╕Å","Engine oils and fluids"]].map(([icon,item])=>(
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:10, marginBottom:"1.5rem" }}>
            {[["≡ƒÅ¬","Pickup","Collect from shop"],["≡ƒÜÜ","Delivery","CCC riders deliver"],["Γ£à","Verified shops","All shops checked"],["≡ƒÆ│","Secure pay","M-Pesa or card"]].map(([icon,title,desc])=>(
          ...
          <button onClick={()=>navigate("/auth")} style={btnOrange}>Browse parts ΓåÆ</button>
        </div>
      </div>
          ...
          {faqs.map((faq,i)=>(
            <div key={i} className="faq-row" onClick={()=>setOpenFaq(openFaq===i?null:i)}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12 }}>
                ...
                <span style={{ fontSize:22, color:"#e6821e", flexShrink:0, fontWeight:300 }}>{openFaq===i?"╬ô├¬├å":"+"}</span>
              </div>
      ...
      <div style={{ background:"rgba(0,0,0,0.90)", padding:"4rem 1.25rem", textAlign:"center", position:"relative", overflow:"hidden", zIndex:1 }}>
        ...
        <div style={{ maxWidth:500, margin:"0 auto", position:"relative", zIndex:2 }}>
          ...
          <div style={{ display:"flex", gap:10, justifyContent:"center", flexWrap:"wrap" }}>
            <button onClick={()=>navigate("/auth")} style={btnOrange}>≡ƒÜù Get started free</button>
            <button onClick={()=>navigate("/auth")} style={{ ...btnOutline, color:"#fff", borderColor:"#555" }}>Sign in ΓåÆ</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ background:"rgba(17,17,17,0.95)", padding:"2rem 1.25rem", borderTop:"1px solid #222", position:"relative", zIndex:1 }}>
        <div style={{ maxWidth:800, margin:"0 auto" }}>
          ...
          <p style={{ fontSize:12, color:"#666", marginBottom:"1rem", lineHeight:1.6 }}>≡ƒç░≡ƒç¬ Nairobi's most trusted automotive platform</p>
          ...
          <div style={{ fontSize:11, color:"#444" }}>Γö¼ΓîÉ 2026 Car Care Connect Γö¼Γòû Nairobi, Kenya Γö¼Γòû Payments secured by Pesapal Γö¼Γòû Regulated by Central Bank of Kenya</div>
        </div>
      </div>
