...
export default function ProviderDashboard() {
  ...
  useEffect(() => {
    ...
    const sub = supabase.channel("prov-dash-v2")
      ...
      .on("postgres_changes", { event:"INSERT", schema:"public", table:"orders", filter:`provider_id=eq.${user.id}` }, payload => { load(); toast.success(`New order received! 📦 Order #${payload.new.order_number||""}`, { duration:10000 }) })
      .on("postgres_changes", { event:"UPDATE", schema:"public", table:"orders", filter:`provider_id=eq.${user.id}` }, () => load())
      .subscribe()
