...
import { DiscoverIcon, VehicleIcon, ProfileIcon, ClockIcon, CheckCircleIcon, CheckIcon, CameraIcon } from "../../lib/cccIcons"
...
export default function WashQueue() {
  ...
  return (
    <div>
      <div style={{fontFamily:"Syne",fontSize:18,fontWeight:800,color:"#000",marginBottom:4,display:"flex",alignItems:"center",gap:8}}><DiscoverIcon size={20} color="#000"/> Active Wash Queue</div>
      <div style={{fontSize:12,color:"#777",marginBottom:"1.5rem"}}>{bookings.length} active booking{bookings.length!==1?"s":""}</div>
      {bookings.length===0&&<div style={{textAlign:"center",padding:"3rem",background:"#f5f5f5",borderRadius:12}}><div style={{marginBottom:10,display:"flex",justifyContent:"center"}}><VehicleIcon size={40} color="#bbb"/></div><div style={{fontSize:14,color:"#888"}}>No active bookings right now</div></div>}
      {bookings.map(b=>(
        <div key={b.id} style={{background:"#fff",border:`2px solid ${STATUS_COLORS[b.status]||"#eee"}30`,borderRadius:14,padding:"1.25rem",marginBottom:12,borderLeft:`4px solid ${STATUS_COLORS[b.status]||"#eee"}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
            <div>
              ...
              <div style={{fontSize:12,color:"#666",display:"flex",alignItems:"center",gap:4}}><ProfileIcon size={12} color="#666"/> {b.profiles?.first_name} {b.profiles?.last_name}</div>
              {b.vehicles&&<div style={{fontSize:12,color:"#666",display:"flex",alignItems:"center",gap:4}}><VehicleIcon size={12} color="#666"/> {b.vehicles.color} {b.vehicles.make} {b.vehicles.model} · {b.vehicles.license_plate}</div>}
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>
            ...
            {b.status==="confirmed"&&<button onClick={()=>updateStatus(b.id,"in-progress")} style={{background:"#8b5cf6",border:"none",borderRadius:8,color:"#fff",fontSize:12,fontWeight:600,padding:"7px 14px",cursor:"pointer"}}><span style={{display:"inline-flex",alignItems:"center",gap:4}}><DiscoverIcon size={13} color="#fff"/> Start Wash</span></button>}
            {b.status==="in-progress"&&<button onClick={()=>updateStatus(b.id,"completed")} style={{background:"#1d9e75",border:"none",borderRadius:8,color:"#fff",fontSize:12,fontWeight:600,padding:"7px 14px",cursor:"pointer"}}><span style={{display:"inline-flex",alignItems:"center",gap:4}}><CheckCircleIcon size={13} color="#fff"/> Complete</span></button>}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
            {["before","after"].map(type=>(
              <div key={type} style={{background:"#f5f5f5",borderRadius:8,padding:"0.75rem",textAlign:"center"}}>
                <div style={{fontSize:11,color:"#666",marginBottom:6,display:"flex",alignItems:"center",justifyContent:"center",gap:4}}><CameraIcon size={11} color="#666"/> {type==="before"?"Before":"After"} photo</div>
