    ...
    <div>
      ...
      <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1rem", marginBottom:"1rem" }}>
        <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:12 }}>
          ...
          <div>
            <div style={{ fontFamily:"Syne", fontSize:16, fontWeight:800, color:"#f0ede6" }}>{trackingMechanic.first_name} {trackingMechanic.last_name}</div>
            <div style={{ fontSize:12, color:"#555" }}>🔧 {trackingMechanic.specialization}</div>
            {trackingMechanic.phone&&<div style={{ fontSize:12, color:"#555" }}>📞 {trackingMechanic.phone}</div>}
          </div>
        ...
        {trackingMechanic.current_latitude?(
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:12 }}>
            <div style={{ background:"#0f0f0f", borderRadius:8, padding:"0.75rem" }}>
              <div style={{ fontSize:10, color:"#555", marginBottom:2 }}>Latitude</div>
              <div style={{ fontSize:13, color:"#1d9e75", fontFamily:"monospace" }}>{trackingMechanic.current_latitude?.toFixed(6)}</div>
            </div>
            <div style={{ background:"#0f0f0f", borderRadius:8, padding:"0.75rem" }}>
              <div style={{ fontSize:10, color:"#555", marginBottom:2 }}>Longitude</div>
              <div style={{ fontSize:13, color:"#1d9e75", fontFamily:"monospace" }}>{trackingMechanic.current_longitude?.toFixed(6)}</div>
            </div>
          </div>
        ):(
          <div style={{ background:"#0f0f0f", borderRadius:8, padding:"0.75rem", marginBottom:12, fontSize:12, color:"#444", textAlign:"center" }}>
            Waiting for mechanic to share location...
          </div>
        )}

        <div ref={mapRef} style={{ height:300, borderRadius:10, overflow:"hidden", background:"#1a1a1a" }}>
          {!trackingMechanic.current_latitude&&(
            <div style={{ height:"100%", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:8 }}>
              <div style={{ fontSize:28 }}>📍</div>
              <div style={{ fontSize:12, color:"#555" }}>No location shared yet</div>
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1rem" }}>
        <div style={{ fontFamily:"Syne", fontSize:13, fontWeight:700, marginBottom:"0.75rem", color:"#f0ede6" }}>Location history</div>
  ...
  return (
    <div>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"1rem" }}>
        <div style={{ fontFamily:"Syne", fontSize:16, fontWeight:800, color:"#f0ede6" }}>Your mechanics team</div>
...
function LocationHistory({ mechanicId }) {
  ...
  if (!history.length) return <div style={{ fontSize:12, color:"#444" }}>No location history</div>
  return history.map((h,i)=>(
    <div key={h.id} style={{ display:"flex", justifyContent:"space-between", padding:"6px 0", borderBottom:"1px solid #1a1a1a", fontSize:11 }}>
      <span style={{ color:"#555", fontFamily:"monospace" }}>{h.latitude.toFixed(5)}, {h.longitude.toFixed(5)}</span>
      <span style={{ color:"#444" }}>{new Date(h.recorded_at).toLocaleTimeString()}</span>
    </div>
  ))
}
