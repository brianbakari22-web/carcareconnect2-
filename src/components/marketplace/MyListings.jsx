            ...
            <div key={l.id} style={{ background:"#111", border:"1px solid "+(SC[l.status]||"#1e1e1e")+"30", borderRadius:12, overflow:"hidden", marginBottom:12 }}>
              <div style={{ height:140, background:"#1a1a1a", position:"relative", overflow:"hidden" }}>
                ...
                <div style={{ position:"absolute", top:8, left:8, display:"flex", gap:4, flexWrap:"wrap" }}>
                  ...
                  {l.is_featured&&<span style={{ fontSize:10, padding:"2px 8px", borderRadius:10, background:"#e6821e", color:"#fff" }}>⭐</span>}
                  {l.is_inspected&&<span style={{ fontSize:10, padding:"2px 8px", borderRadius:10, background:"#1d9e75", color:"#fff" }}>✓</span>}
                </div>
              ...
              <div style={{ padding:"0.75rem" }}>
                ...
                {l.listing_type==="vehicle"&&<div style={{ fontSize:11, color:"#888", marginBottom:3 }}>{[l.make,l.model,l.year,l.mileage?Number(l.mileage).toLocaleString()+"km":null].filter(Boolean).join(" · ")}</div>}
                <div style={{ fontSize:11, color:"#555", marginBottom:8 }}>📍 {l.city||"—"} · 👁 {l.views||0} views · {new Date(l.created_at).toLocaleDateString()}</div>
                {l.status==="pending"&&l.listing_type==="vehicle"&&l.inspection_status!=="passed"&&(
                  <div style={{ background:"#1a1208", border:"1px solid #e6821e30", borderRadius:8, padding:"0.6rem", marginBottom:8 }}>
                    ...
                    <button onClick={()=>setShowInspection(showInspection===l.id?null:l.id)}
                      ...
                      🔍 {showInspection===l.id?"Close":"Schedule & Pay Inspection (KES 500)"}
                    </button>
                ...
                {photoListing===l.id&&(
                  <div style={{ marginBottom:8 }}>
                    <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:8 }}>
                      {listingPhotos.map((p,i)=>(
                        <img key={p.id} src={p.photo_url} alt="" style={{ width:64, height:64, objectFit:"cover", borderRadius:8, border:p.is_primary?"2px solid #e6821e":"1px solid #333" }}/>
                      ))}
                ...
                <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                  ...
                  {l.status!=="sold"&&<button onClick={()=>deleteListing(l.id)} style={{ background:"none", border:"1px solid #e24b4a30", borderRadius:7, color:"#e24b4a", fontSize:11, padding:"6px 12px", cursor:"pointer" }}>🗑️ Delete</button>}
                </div>
              </div>
            </div>
          ))}
