...
export default function ChatWindow({ bookingId, otherUserId, otherUserName, onClose }) {
  const { user } = useAuth()
  const { t } = useLanguage()
    ...
    const channel = supabase.channel(`chat-${bookingId}`)
      .on("postgres_changes", {
        ...
        setMessages(prev => {
          ...
          if (exists) {
            return prev.map(m => m._tempId === payload.new.id ? { ...payload.new } : m)
          }
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", height:"100%", background:"#0f0f0f", borderRadius:12, border:"1px solid #2a2a2a", overflow:"hidden" }}>
