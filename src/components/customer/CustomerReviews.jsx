...
export default function CustomerReviews() {
  ...
  const inp = { width:"100%", background:"#111", border:"1px solid #222", borderRadius:8, padding:"10px 12px", color:"#f0ede6", fontSize:13, outline:"none", fontFamily:"'DM Sans',sans-serif", resize:"vertical" }
  ...
  return (
    <div>
      ...
      {tab==="pending"&&!reviewing&&(
        <div>
          {pendingReview.length===0&&!loading&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>{t("noDataYet")}</div>}
          {pendingReview.map(b=>(
      ...
      {tab==="pending"&&reviewing&&(
        <div>
          ...
          <div style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:12, padding:"1.25rem" }}>
            <div style={{ fontFamily:"Syne", fontSize:15, fontWeight:800, color:"#f0ede6", marginBottom:2 }}>{reviewing.service_name}</div>
            <div style={{ fontSize:11, color:"#555", marginBottom:"1.5rem" }}>{reviewing.booking_date}</div>
      ...
      {tab==="submitted"&&(
        <div>
          {myReviews.length===0&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>{t("noDataYet")}</div>}
          {myReviews.map(r=>(
            <div key={r.id} style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:10, padding:"1rem", marginBottom:8 }}>
              ...
              {r.provider_review&&<div style={{ fontSize:12, color:"#888", lineHeight:1.5, marginBottom:8 }}>"{r.provider_review}"</div>}
              {r.provider_response&&(
                <div style={{ background:"#0f0f0f", border:"1px solid #222", borderRadius:8, padding:"0.75rem", marginTop:8 }}>
                  <div style={{ fontSize:10, color:"#555", marginBottom:4 }}>{t("language")==="sw"?"Jibu la mtoa huduma":"Provider reply"}</div>
                  <div style={{ fontSize:12, color:"#aaa", lineHeight:1.5 }}>{r.provider_response}</div>
                </div>
              )}
            </div>
          ))}
