...
export default function ProviderStorefront({ provider, onClose, onBook }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.9)", zIndex:300, overflowY:"auto" }}>
      <div style={{ maxWidth:600, margin:"0 auto", background:"#0d0d0d", minHeight:"100vh" }}>
        
        {/* Header */}
        <div style={{ position:"sticky", top:0, zIndex:10, background:"#0d0d0d", padding:"0.75rem 1rem", display:"flex", justifyContent:"space-between", alignItems:"center", borderBottom:"1px solid #1e1e1e" }}>
          <button onClick={onClose} style={{ background:"none", border:"none", color:"#888", fontSize:22, cursor:"pointer", padding:"0 8px" }}>←</button>
          <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, color:"#f0ede6" }}>{provider.business_name||`${provider.first_name} ${provider.last_name}`}</div>
          <div style={{ width:40 }}/>
        </div>
        ...
        <div style={{ padding:"1.25rem" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
            <div>
              ...
              <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center" }}>
                ...
                {provider.city&&<span style={{ fontSize:11, color:"#555" }}>📍 {provider.city}</span>}
              </div>
            </div>
            <div style={{ textAlign:"right" }}>
              <div style={{ fontFamily:"Syne", fontSize:22, fontWeight:800, color:"#e6821e" }}>{avgRating}</div>
              <div style={{ fontSize:10, color:"#555" }}>⭐ {reviews.length} reviews</div>
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:8, marginBottom:"1.5rem" }}>
            ...
            <button onClick={()=>{ onClose(); navigate("/dashboard/chat") }}
              style={{ background:"#111", border:"1px solid #333", borderRadius:10, color:"#888", fontFamily:"Syne,sans-serif", fontSize:13, fontWeight:700, padding:"12px 16px", cursor:"pointer" }}>
              💬 Chat
            </button>
          </div>
          ...
          {tab==="about"&&(
            <div>
              {provider.bio&&<p style={{ fontSize:13, color:"#888", lineHeight:1.7, marginBottom:12 }}>{provider.bio}</p>}
              <div style={{ background:"#111", borderRadius:10, padding:"1rem" }}>
                {[
                  ...
                  <div key={f.label} style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:"1px solid #1a1a1a" }}>
                    ...
                    <span style={{ fontSize:12, color:"#f0ede6" }}>{f.value}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {tab==="services"&&(
            <div>
              {services.length===0&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No services listed yet</div>}
              {services.map(s=>(
                <div key={s.id} style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:10, padding:"1rem", marginBottom:8 }}>
                  ...
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                    <div>
                      <div style={{ fontSize:14, fontWeight:600, color:"#f0ede6", marginBottom:2 }}>{s.name}</div>
                      {s.description&&<div style={{ fontSize:11, color:"#666", marginBottom:4 }}>{s.description}</div>}
                      <div style={{ fontSize:11, color:"#555" }}>⏱ {s.duration_minutes||60} min</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
          ...
          {tab==="inventory"&&(
            <div>
              {inventory.length===0&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No products listed yet</div>}
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                {inventory.map(item=>(
                  <div key={item.id} style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:10, padding:"0.75rem", cursor:"pointer" }}
                    ...
                    <div style={{ fontSize:12, fontWeight:600, color:"#f0ede6", marginBottom:2 }}>{item.name}</div>
                    {item.brand&&<div style={{ fontSize:10, color:"#888", marginBottom:2 }}>{item.brand}</div>}
          ...
          {tab==="reviews"&&(
            <div>
              {reviews.length===0&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No reviews yet</div>}
              {reviews.map(r=>(
                <div key={r.id} style={{ background:"#111", border:"1px solid #1e1e1e", borderRadius:10, padding:"1rem", marginBottom:8 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
                    <div style={{ fontSize:12, fontWeight:600, color:"#f0ede6" }}>{r.profiles?.first_name} {r.profiles?.last_name?.[0]}.</div>
                    <div style={{ fontSize:12, color:"#e6821e" }}>{"⭐".repeat(Math.round(r.provider_rating||0))}</div>
                  </div>
                  {r.provider_comment&&<div style={{ fontSize:12, color:"#888", lineHeight:1.6 }}>{r.provider_comment}</div>}
                  <div style={{ fontSize:10, color:"#444", marginTop:4 }}>{new Date(r.created_at).toLocaleDateString()}</div>
                </div>
              ))}
            </div>
          )}
