              ...
              {mechanic?.carries_parts&&(
                <div>
                  ...
                  <div style={{ display:"flex", flexWrap:"wrap", gap:8 }}>
                    {[{k:"battery",l:"🔋 Battery"},{k:"tyre_tools",l:"🛞 Tyre Tools"},{k:"fuel",l:"⛽ Fuel"},{k:"jumper_cables",l:"⚡ Jumper Cables"},{k:"fuses",l:"🔌 Fuses"},{k:"engine_oil",l:"🛢️ Engine Oil"}].map(p=>{ const sel=(mechanic?.parts_carried||[]).includes(p.k); return <div key={p.k} onClick={()=>{ const cur=mechanic?.parts_carried||[]; const upd=sel?cur.filter(x=>x!==p.k):[...cur,p.k]; setMechanic(m=>({...m,parts_carried:upd})); supabase.rpc("update_mechanic_parts",{ p_mechanic_id:mechanic.mechanic_id, p_carries_parts:mechanic?.carries_parts||false, p_parts_carried:upd, p_provider_id:mechanic.provider_id }).then(({error})=>{ if(error) { setMechanic(m=>({...m,parts_carried:cur})); toast.error("Failed to save") } }) }} style={{ padding:"6px 14px", borderRadius:20, border:"1px solid "+(sel?"#8b5cf6":"#ddd"), background:sel?"#8b5cf6":"#f8f8f8", color:sel?"#fff":"#555", fontSize:12, cursor:"pointer", fontWeight:sel?700:400 }}>{p.l}</div> })}
                  </div>
                </div>
              )}
            </div>
