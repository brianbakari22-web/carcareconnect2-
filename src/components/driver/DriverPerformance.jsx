...
export default function DriverPerformance() {
  ...
  return (
    <div>
      <div style={{ fontFamily:"Syne", fontSize:isMobile?16:20, fontWeight:800, color:"#f0ede6", marginBottom:4 }}>Performance</div>
      <div style={{ fontSize:12, color:"#555", marginBottom:"1.5rem" }}>Your driving stats and ratings</div>

      {/* Rating highlight */}
      <div style={{ background:"linear-gradient(135deg,#1a1208,#111)", border:"1px solid #e6821e30", borderRadius:16, padding:"1.5rem", marginBottom:"1.5rem", textAlign:"center" }}>
        ...
        <div style={{ fontSize:12, color:"#555" }}>{vc.icon} {vc.label}</div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:10, marginBottom:"1.5rem" }}>
        {[
          ...
          <div key={s.label} style={{ background:"#111", borderRadius:12, padding:"1rem", border:"1px solid #1e1e1e", textAlign:"center" }}>
      ...
      <div style={{ background:"#111", borderRadius:12, padding:"1rem", marginBottom:"1.5rem", border:"1px solid #1e1e1e" }}>
        <div style={{ fontFamily:"Syne", fontSize:13, fontWeight:700, color:"#f0ede6", marginBottom:12 }}>Performance indicators</div>
        {[
          ...
          <div key={ind.label} style={{ marginBottom:12 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
              <span style={{ fontSize:12, color:"#888" }}>{ind.label}</span>
      ...
      {/* Recent jobs */}
      <div style={{ fontFamily:"Syne", fontSize:14, fontWeight:700, color:"#f0ede6", marginBottom:10 }}>Recent jobs</div>
      {recentJobs.length===0&&<div style={{ color:"#444", fontSize:13, textAlign:"center", padding:"2rem" }}>No jobs yet</div>}
      {recentJobs.map((job,i)=>(
        <div key={i} style={{ background:"#111", borderRadius:10, padding:"0.75rem", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center", border:"1px solid #1e1e1e" }}>
          <div>
            <div style={{ fontSize:13, color:"#f0ede6", marginBottom:2 }}>{job.service_name||"Delivery job"}</div>
            <div style={{ fontSize:11, color:"#555" }}>{new Date(job.created_at).toLocaleDateString()}</div>
          </div>
