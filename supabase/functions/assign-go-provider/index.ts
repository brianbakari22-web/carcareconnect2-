...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!booking.is_emergency || booking.provider_id) {
      return new Response(JSON.stringify({ message: "Booking already assigned or not a GO Service request" }))
    }

    const attempt = (booking.go_attempt_number || 0) + 1
    const MAX_ATTEMPTS = 5

    // Get provider_ids already tried for this booking
    ...
    if (attempt > MAX_ATTEMPTS) {
      // Refund the callout fee - no provider available
      ...
      return new Response(JSON.stringify({ message: "No provider found - refund issued" }))
    }

    // Find providers offering this exact service, online, within radius, not yet tried
    ...
    // Also check broader: other providers offering the SAME category service, if the original listing itself has no other candidates
    ...
    if (!eligible.length) {
      ...
      return new Response(JSON.stringify({ message: "No providers online, will retry" }))
    }
    ...
    if (reqError) {
      ...
      return new Response(JSON.stringify({ error: "Failed to create request", detail: reqError.message }), { status: 500 })
    }
