    ...
    switch(name) {
      ...
      case "send_notification": {
        ...
        if(input.user_id.startsWith("all_")) {
          const role = input.user_id.replace("all_", "").slice(0,-1) // remove 's'
...
2. For DESTRUCTIVE actions (cancel, verify, release payment, send notifications to all users) - ALWAYS ask for confirmation first before executing
3. For READ actions (query, count) - execute immediately and show results
