        ...
        if (txn?.booking_id) {
          ...
          if (!bk) {
            ...
            if (groupOrders && groupOrders.length > 0) {
              await supabase.from("orders").update({ payment_status: "paid", payment_held: true, status: "pending" }).eq("group_order_id", txn.booking_id).eq("status", "pending_payment")
              ...
              if (carListing) {
                const { data: durSetting } = await supabase.from("app_settings").select("value").eq("key","new_car_listing_duration_days").maybeSingle()
                const listingDurationDays = durSetting ? Number(durSetting.value) : 30
                await supabase.from("new_car_listings").update({ listing_fee_paid: true, listing_paid_at: new Date().toISOString(), is_active: true, listing_expires_at: new Date(Date.now() + listingDurationDays*24*60*60*1000).toISOString() }).eq("id", carListing.id)
